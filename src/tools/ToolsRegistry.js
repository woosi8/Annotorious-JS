...
export default class ToolRegistry extends EventEmitter {
	constructor(g, config, env) {
		...
		this._registered = [RubberbandRectTool, RubberbandPolygonTool];

		this.setCurrent(RubberbandPolygonTool);
	}

	listTools = () => this._registered.map((impl) => impl.identifier);

	registerTool = (impl) => {
		// If there's a tool with the same ID, replace
		const id = impl.identifier;
		if (this.listTools().includes(id)) this.unregisterTool(id);
	...
	setCurrent = (toolOrId) => {
		const Tool =
			typeof toolOrId === "string" || toolOrId instanceof String
