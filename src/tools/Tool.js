	...
	enableResponsive = () => {
		if (window.ResizeObserver) {
			this.resizeObserver = new ResizeObserver(() => {
				...
				if (this.onScaleChanged) this.onScaleChanged(this.scale);
			});
...
export default class Tool extends ToolLike {
	...
	attachListeners = ({ mouseMove, mouseUp, dblClick }) => {
		...
		if (dblClick) {
			...
			// document.addEventListener("keydown", this.dblClick);
			document.addEventListener("touchend", this.dblClick);
		}
	...
	detachListeners = () => {
		...
		if (this.mouseUp) document.removeEventListener("mouseup", this.mouseUp);

		if (this.dblClick) document.removeEventListener("dblclick", this.dblClick);
	};
