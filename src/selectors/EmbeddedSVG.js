...
export const toSVGTarget = (shape, image) => {
	...
	let serialized =
		inner.outerHTML || new XMLSerializer().serializeToString(inner);
...
export const svgArea = (annotation) => {
	...
	if (nodeName === "polygon") return polygonArea(shape);
	else if (nodeName === "circle") return circleArea(shape);
	else if (nodeName === "ellipse") return ellipseArea(shape);
	else if (nodeName == "path") return pathArea(shape);
	else throw `Unsupported SVG shape type: ${nodeName}`;
};
