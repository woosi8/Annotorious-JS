...
export class Annotorious {
	...
	handleAnnotationCreated = (annotation, overrideId) =>
		...
		this._emitter.emit(
			"createAnnotation",
			annotation.underlying.target.selector.value
		);
	...
	handleSelectionStarted = (pt) => this._emitter.emit("startSelection", pt);
	...
	addAnnotation = (annotation, readOnly) =>
		this._app.current.addAnnotation(
			new WebAnnotation(annotation, { readOnly })
		);

	addDrawingTool = (plugin) => this._app.current.addDrawingTool(plugin);

	cancelSelected = () => this._app.current.cancelSelected();

	clearAnnotations = () => this.setAnnotations([]);

	clearAuthInfo = () => (this._env.user = null);
	...
	getSelectedImageSnippet = () => this._app.current.getSelectedImageSnippet();

	listDrawingTools = () => this._app.current.listDrawingTools();
	...
	once = (event, handler) => this._emitter.once(event, handler);
	...
	removeDrawingTool = (id) => this._app.current.removeDrawingTool(id);

	saveSelected = () => this._app.current.saveSelected();
	...
	setAuthInfo = (authinfo) => (this._env.user = authinfo);

	setDrawingTool = (shape) => this._app.current.setDrawingTool(shape);

	setServerTime = (timestamp) => this._env.setServerTime(timestamp);

	setVisible = (visible) => this._app.current.setVisible(visible);
	...
	updateSelected = (annotation, saveImmediately) => {
		...
		if (updated) this._app.current.updateSelected(updated, saveImmediately);
	};
